...
public class WindowManager: NSObject, NSWindowDelegate {
    ...
    public func setTitleBarStyle(_ args: [String: Any]) {
        ...
        // Safely access the title bar view — the button superview hierarchy can be
        // nil on macOS 15+ (Sequoia/macOS 26) when the window chrome was restructured.
        if let closeButton = mainWindow.standardWindowButton(.closeButton),
           let buttonSuperview = closeButton.superview,
           let titleBarView = buttonSuperview.superview {
